...
# Setup directories
raw_videos_dir, musics_dir, transcripts_dir, subtitle_styles_dir, shorts_dir = setup_dirs()
...
def config_name_component():
    ...
    st.caption("Give your configuration a descriptive name (e.g., 'Classic_White', 'Bold_Yellow', 'Minimal_Black')")
...
def save_subtitle_config_component():
    if st.button("💾 Save Configuration", type="primary", use_container_width=True):
        if st.session_state.config_name_input and st.session_state.config_name_input.strip():
            if save_subtitle_config(st.session_state.config_name_input.strip(), st.session_state.current_config):
                st.success(f"Configuration '{st.session_state.config_name_input.strip()}' saved successfully!")
                st.info("💡 **Next Steps**: Your configuration is now saved and can be used in video generation. Go to the Manual Generation or Automatic Generation pages to apply this subtitle style to your videos!")
                st.rerun()
        else:
            st.error("Please enter a configuration name!")
    
    # Add save information
    st.caption(f"💾 **Save Info**: Configurations are saved as YAML files in the '{subtitle_styles_dir}' folder and are automatically available for use in video generation.")
...
def ass_parameters_component():
    ...
    with col1a:
        ...
        if ass_params['PrimaryColour'].startswith('#'):
            ...
            ass_params['PrimaryColour'] = f"&H00{hex_color[4:6].upper()}{hex_color[2:4].upper()}{hex_color[0:2].upper()}"
    ...
    with col2a:
        ...
        if ass_params['SecondaryColour'].startswith('#'):
            ...
            ass_params['SecondaryColour'] = f"&H00{hex_color[4:6].upper()}{hex_color[2:4].upper()}{hex_color[0:2].upper()}"
    ...
    with col3a:
        ...
        if ass_params['OutlineColour'].startswith('#'):
            ...
            ass_params['OutlineColour'] = f"&H00{hex_color[4:6].upper()}{hex_color[2:4].upper()}{hex_color[0:2].upper()}"
    ...
    with col4a:
        ...
        if ass_params['BackColour'].startswith('#'):
            ...
            ass_params['BackColour'] = f"&H64{hex_color[4:6]}{hex_color[2:4]}{hex_color[0:2]}"
    ...
    with col1b:
        ass_params['fontname'] = st.selectbox(
            ...
            index=['Impact', 'Arial', 'Helvetica', 'Times New Roman', 'Verdana'].index(ass_params.get('fontname', 'Impact'))
        )
    ...
    with col4b:
        ...
        selected_alignment = st.selectbox(
            ...
            index=list(alignment_options.keys()).index(ass_params.get('Alignment', 'bottom-middle'))
        )
...
def special_effects_component():
    ...
    with col1e:
        special_effects['zoom_in'] = st.checkbox(
            "Zoom In Effect",
            value=special_effects.get('zoom_in', False)
        )
    ...
    with col2e:
        special_effects['box_highlight'] = st.checkbox(
            "Box Highlight",
            value=special_effects.get('box_highlight', False)
        )
    ...
    with col3e:
        special_effects['color_highlight'] = st.checkbox(
            "Color Highlight",
            value=special_effects.get('color_highlight', False)
        )
    ...
    with col4e:
        special_effects['karaoke_highlight'] = st.checkbox(
            "Karaoke Highlight",
            value=special_effects.get('karaoke_highlight', False)
        )
...
def subtitles_parameters_component():
    ...
    with col3f:
        ...
        subtitles_params['upper_case'] = st.checkbox(
            ...
            value=subtitles_params.get('upper_case', False)
        )
